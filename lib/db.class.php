<?php
  ...
  class DB
  {
    ...
    include './config.php';
    $this->db = $this->db_conn($config['database'], $config['host'], $config['username'], $config['password']);
  }

    /** Connect to a mysqli database to be able to use the methods below.
      */
    function db_conn($base, $server, $user, $pass)
    {
      $this->mtStart    = $this->getMicroTime();
      $this->nbQueries  = 0;
      $this->lastResult = NULL;
     $myconnection = mysqli_connect($server, $user, $pass,$base);
